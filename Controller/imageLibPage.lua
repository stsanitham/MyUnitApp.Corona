...
local function onRowRender_ImageLib( event )
 ...
 local row = event.row
    ...
    text = display.newText(row,List_array[row.index].ImageFileName,0,0,native.systemFont,16)
...
function scene:show( event )
	...
	if phase == "will" then
		...
		elseif phase == "did" then
			...
			List_array = Get_AllMyunitappImage()
		...
		for i = 1, #List_array do
		    ...
		    Image_Lib_list:insertRow{ rowHeight = 45,rowColor = 
		    {
		    default = { 1, 1, 1, 0 },
		    over={ 1, 0.5, 0, 0 },
